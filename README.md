...
The shared 0.5×–2.0× acceleration factor smoothly reshapes both curves without changing their start or maximum-temperature endpoints. FanBar enters manual mode only when its curve target is higher than the target currently reported by macOS. A manual session never lowers its target, and every ten seconds FanBar briefly restores automatic control so macOS can reassert demand from sensors FanBar does not use. Physical RPM increases still pass through the slew limiter; 90°C emergencies request maximum speed immediately.
